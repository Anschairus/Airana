...
import { NpcService, AuthenticationService } from "../../_services";
...
@Component({
  selector: "selected-npc",
  templateUrl: "./selected-npc.component.html",
  styleUrls: ["./selected-npc.component.scss"]
})
export class SelectedNpcComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit() {
    ...
    //request id's data
    this.npcService.onSelect(this.surname, this.name).subscribe((data: CharacterTab) => {
    ...
    this.characterTabForm = this.npcService.onSelect(this.surname, this.name).pipe(
  ...
  characterImg(characterTab: CharacterTab) {
    if (characterTab.image.main.url != "" || characterTab.image.main.url != null) {
      return "assets/img/npc/" + characterTab.image.main.url;
...
if (item.link.image){
  ...
  onSubmit() {
    ...
    if (this.form.valid) {//this.loading = true;
      this.npcService.update(this.form.value.surname, this.form.value.name, this.form.value)
        .subscribe(
          data => {
